...
void compare_3d_tensors(
    ...
    if (error_count == 0) {
        std::cout << "\n✅ 测试通过: C++split_3d结果与PyTorch在数值上完全一致!" << std::endl;
    } else {
...
int main() {
    std::cout << "=== Split_3D 算子详细数值对比测试 ===" << std::endl;
    
    try {
        // 测试案例1: 沿axis=0均等分割
        std::cout << "\n=== 测试案例1: 沿axis=0均等分割 ===" << std::endl;
        
        auto input1 = load_3d_tensor("split3d_case1_input.npy");
        auto params1 = load_split_params("split3d_case1_params.npy");
        ...
        // C++分割
        auto cpp_results1 = split_3d(input1, axis1, num_splits1);
        std::cout << "C++分割结果数量: " << cpp_results1.size() << std::endl;
        
        // 逐个比较每个分割结果
        for (int i = 0; i < num_splits1; i++) {
            auto pytorch_result = load_3d_tensor("split3d_case1_output" + std::to_string(i+1) + ".npy");
            std::cout << "\n--- 分割片段 " << (i+1) << " ---" << std::endl;
            std::cout << "C++结果形状: [" << cpp_results1[i].size() << ", " << cpp_results1[i][0].size() << ", " << cpp_results1[i][0][0].size() << "]" << std::endl;
            std::cout << "PyTorch结果形状: [" << pytorch_result.size() << ", " << pytorch_result[0].size() << ", " << pytorch_result[0][0].size() << "]" << std::endl;
            
            compare_3d_tensors(cpp_results1[i], pytorch_result, "沿axis=0均等分割-片段" + std::to_string(i+1));
        }
        
        // 测试案例2: 沿axis=1均等分割
        std::cout << "\n=== 测试案例2: 沿axis=1均等分割 ===" << std::endl;
        
        auto input2 = load_3d_tensor("split3d_case2_input.npy");
        auto params2 = load_split_params("split3d_case2_params.npy");
        ...
        auto cpp_results2 = split_3d(input2, axis2, num_splits2);
        std::cout << "C++分割结果数量: " << cpp_results2.size() << std::endl;
        
        for (int i = 0; i < num_splits2; i++) {
            auto pytorch_result = load_3d_tensor("split3d_case2_output" + std::to_string(i+1) + ".npy");
            std::cout << "\n--- 分割片段 " << (i+1) << " ---" << std::endl;
            
            compare_3d_tensors(cpp_results2[i], pytorch_result, "沿axis=1均等分割-片段" + std::to_string(i+1));
        }
        
        // 测试案例3: 沿axis=2均等分割
        std::cout << "\n=== 测试案例3: 沿axis=2均等分割 ===" << std::endl;
        
        auto input3 = load_3d_tensor("split3d_case3_input.npy");
        auto params3 = load_split_params("split3d_case3_params.npy");
        ...
        auto cpp_results3 = split_3d(input3, axis3, num_splits3);
        std::cout << "C++分割结果数量: " << cpp_results3.size() << std::endl;
        
        for (int i = 0; i < num_splits3; i++) {
            auto pytorch_result = load_3d_tensor("split3d_case3_output" + std::to_string(i+1) + ".npy");
            std::cout << "\n--- 分割片段 " << (i+1) << " ---" << std::endl;
            
            compare_3d_tensors(cpp_results3[i], pytorch_result, "沿axis=2均等分割-片段" + std::to_string(i+1));
        }
        
        // 测试案例4: 不均等分割
        std::cout << "\n=== 测试案例4: 沿axis=1不均等分割 ===" << std::endl;
        
        auto input4 = load_3d_tensor("split3d_case4_input.npy");
        auto params4 = load_split_params("split3d_case4_params.npy");
        ...
        auto cpp_results4 = split_3d(input4, axis4, 0, split_sizes4);
        std::cout << "C++分割结果数量: " << cpp_results4.size() << std::endl;
        
        for (size_t i = 0; i < split_sizes4.size(); i++) {
            auto pytorch_result = load_3d_tensor("split3d_case4_output" + std::to_string(i+1) + ".npy");
            std::cout << "\n--- 分割片段 " << (i+1) << " ---" << std::endl;
            
            compare_3d_tensors(cpp_results4[i], pytorch_result, "沿axis=1不均等分割-片段" + std::to_string(i+1));
        }
        
        // 测试案例5: 边界测试
        std::cout << "\n=== 测试案例5: 沿axis=0分成2份 ===" << std::endl;
        
        auto input5 = load_3d_tensor("split3d_case5_input.npy");
        auto params5 = load_split_params("split3d_case5_params.npy");
        ...
        auto cpp_results5 = split_3d(input5, axis5, num_splits5);
        std::cout << "C++分割结果数量: " << cpp_results5.size() << std::endl;
        
        for (int i = 0; i < num_splits5; i++) {
            auto pytorch_result = load_3d_tensor("split3d_case5_output" + std::to_string(i+1) + ".npy");
            std::cout << "\n--- 分割片段 " << (i+1) << " ---" << std::endl;
            
            compare_3d_tensors(cpp_results5[i], pytorch_result, "边界测试-片段" + std::to_string(i+1));
        }
        
        std::cout << "\n=== Split_3D 测试完成 ===" << std::endl;
        std::cout << "注意: 测试进行了逐元素的数值对比，确保C++split_3d实现与PyTorch的数学一致性" << std::endl;
