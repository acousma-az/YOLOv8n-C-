...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block21_cv2(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [256, 384, 1, 1]
        ...
        for (int ic = 0; ic < 384; ic++) {
            for (int kh = 0; kh < 1; kh++) {
                for (int kw = 0; kw < 1; kw++) {
                    weight[oc][ic][kh][kw] = model_21_cv2_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_21_cv2_conv_bias, model_21_cv2_conv_bias + 256);
