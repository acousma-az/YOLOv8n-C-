...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block15_cv1(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [64, 192, 1, 1]
        ...
        for (int ic = 0; ic < 192; ic++) {
            for (int kh = 0; kh < 1; kh++) {
                for (int kw = 0; kw < 1; kw++) {
                    weight[oc][ic][kh][kw] = model_15_cv1_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_15_cv1_conv_bias, model_15_cv1_conv_bias + 64);
