...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block5(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [128, 64, 3, 3]
        ...
        for (int ic = 0; ic < 64; ic++) {
            for (int kh = 0; kh < 3; kh++) {
                for (int kw = 0; kw < 3; kw++) {
                    weight[oc][ic][kh][kw] = model_5_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_5_conv_bias, model_5_conv_bias + 128);
