...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block4_cv1(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [64, 64, 1, 1] - 更新为1x1卷积核
        ...
        for (int ic = 0; ic < 64; ic++) {
            for (int kh = 0; kh < 1; kh++) {
                for (int kw = 0; kw < 1; kw++) {
                    weight[oc][ic][kh][kw] = model_4_cv1_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_4_cv1_conv_bias, model_4_cv1_conv_bias + 64);
    std::vector<int> stride = {1, 1}; // 更新stride为1
    std::vector<int> padding = {0, 0}; // 更新padding为0
