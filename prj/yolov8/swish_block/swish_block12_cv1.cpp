...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block12_cv1(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [128, 384, 1, 1]
    std::vector<std::vector<std::vector<std::vector<float>>>> weight(
        128, std::vector<std::vector<std::vector<float>>>(
            384, std::vector<std::vector<float>>(
        ...
        for (int ic = 0; ic < 384; ic++) {
            for (int kh = 0; kh < 1; kh++) {
                for (int kw = 0; kw < 1; kw++) {
                    weight[oc][ic][kh][kw] = model_12_cv1_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_12_cv1_conv_bias, model_12_cv1_conv_bias + 128);
