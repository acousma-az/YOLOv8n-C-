...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block2(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // 将1D权重数组转换为4D张量 [32, 32, 1, 1] - 更新为1x1卷积核
            ...
            for (int kh = 0; kh < 1; kh++) {
                for (int kw = 0; kw < 1; kw++) {
                    if (idx < expected_size) {  // 添加边界检查
                        weight[oc][ic][kh][kw] = model_2_cv1_conv_weight[idx++];
                    }
                }
            }
        }
    }
    std::vector<float> bias(model_2_cv1_conv_bias, model_2_cv1_conv_bias + 32);
