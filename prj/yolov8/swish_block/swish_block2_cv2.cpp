...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block2_cv2(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [32, 48, 1, 1] - 更新为1x1卷积核
    ...
    // 检查权重数组大小，避免越界访问
    int expected_size = 32 * 48 * 1 * 1; // 1024
            ...
            for (int kh = 0; kh < 1; kh++) {
                for (int kw = 0; kw < 1; kw++) {
                    if (idx < expected_size) {  // 添加边界检查
                        weight[oc][ic][kh][kw] = model_2_cv2_conv_weight[idx++];
                    }
                }
            }
        }
    }
    std::vector<float> bias(model_2_cv2_conv_bias, model_2_cv2_conv_bias + 32);
