#include "swish_block22_cv2_0_1.h"
...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block22_cv2_0_1(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [64, 64, 3, 3]
        ...
        for (int ic = 0; ic < 64; ic++) {
            for (int kh = 0; kh < 3; kh++) {
                for (int kw = 0; kw < 3; kw++) {
                    weight[oc][ic][kh][kw] = model_22_cv2_0_1_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_22_cv2_0_1_conv_bias, model_22_cv2_0_1_conv_bias + 64);
