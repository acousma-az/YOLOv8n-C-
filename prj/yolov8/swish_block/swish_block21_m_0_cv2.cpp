#include "swish_block21_m_0_cv2.h"
...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block21_m_0_cv2(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [128, 128, 3, 3]
        ...
        for (int ic = 0; ic < 128; ic++) {
            for (int kh = 0; kh < 3; kh++) {
                for (int kw = 0; kw < 3; kw++) {
                    weight[oc][ic][kh][kw] = model_21_m_0_cv2_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_21_m_0_cv2_conv_bias, model_21_m_0_cv2_conv_bias + 128);
