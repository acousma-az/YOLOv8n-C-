...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block9_cv1(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [128, 256, 1, 1]
        ...
        for (int ic = 0; ic < 256; ic++) {
            for (int kh = 0; kh < 1; kh++) {
                for (int kw = 0; kw < 1; kw++) {
                    weight[oc][ic][kh][kw] = model_9_cv1_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_9_cv1_conv_bias, model_9_cv1_conv_bias + 128);
