        ...
        for (int ic = 0; ic < 512; ic++) {
            for (int kh = 0; kh < 1; kh++) {
                for (int kw = 0; kw < 1; kw++) {
                    weight[oc][ic][kh][kw] = model_9_cv2_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_9_cv2_conv_bias, model_9_cv2_conv_bias + 256);
