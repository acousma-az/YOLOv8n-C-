#include "swish_block2_m_0_cv2.h"
...
std::vector<std::vector<std::vector<std::vector<float>>>> swish_block2_m_0_cv2(
    const std::vector<std::vector<std::vector<std::vector<float>>>>& input
) {
    // [16, 16, 3, 3]
        ...
        for (int ic = 0; ic < 16; ic++) {
            for (int kh = 0; kh < 3; kh++) {
                for (int kw = 0; kw < 3; kw++) {
                    weight[oc][ic][kh][kw] = model_2_m_0_cv2_conv_weight[idx++];
                }
            }
        }
    }
    std::vector<float> bias(model_2_m_0_cv2_conv_bias, model_2_m_0_cv2_conv_bias + 16);
