#include <vector>
#include "../swish_block/swish_block8_m_0_cv1.h"
#include "../swish_block/swish_block8_m_0_cv2.h"
#include "../operator/split.h"
#include "../operator/concat.h"
#include "complex_module1.h"

// 复杂模块函数 - 处理输入张量[batch, 256, 20, 20]
std::vector<std::vector<std::vector<std::vector<float>>>> complex_module1(
    ...
    // 第一步：将输入分成两份，每份128通道
    std::vector<int> split_sizes = {128, 128}; // 将256通道分为两份，每份128通道
    auto split_outputs = split(input, 1, 0, split_sizes);
    // split_outputs[0]和split_outputs[1]尺寸: [batch, 128, 20, 20]
    
    // 第一份直接进入concat
    auto first_branch = split_outputs[0];  // 尺寸: [batch, 128, 20, 20]
    
    // 第二份有三股路径（每股都是128通道的完整副本）
    // 第一股直接进入concat
    auto second_branch_first = split_outputs[1];  // 尺寸: [batch, 128, 20, 20]
    
    // 第二股直接进入add
    auto second_branch_second = split_outputs[1];  // 尺寸: [batch, 128, 20, 20]

    // 第三股先后经过两个swish block，然后进入add
    auto second_branch_third = split_outputs[1];  // 尺寸: [batch, 128, 20, 20]
    auto swish_output1 = swish_block8_m_0_cv1(second_branch_third);  // 尺寸: [batch, 128, 20, 20]
    auto swish_output2 = swish_block8_m_0_cv2(swish_output1);  // 尺寸: [batch, 128, 20, 20]
    ...
    std::vector<std::vector<std::vector<std::vector<std::vector<float>>>>> to_concat = {
        first_branch,           // 尺寸: [batch, 128, 20, 20]
        second_branch_first,    // 尺寸: [batch, 128, 20, 20] 
        add_result              // 尺寸: [batch, 128, 20, 20]
    };
    auto final_output = concat(to_concat, 1);  // 输出尺寸: [batch, 384, 20, 20]
    
    return final_output;  // 最终输出尺寸: [batch, 384, 20, 20]
}
