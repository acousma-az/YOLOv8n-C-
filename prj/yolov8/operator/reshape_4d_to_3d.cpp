...
std::vector<std::vector<std::vector<float>>> reshape_4d_to_3d(
    ...
    size_t channels = (batch > 0) ? input[0].size() : 0;
    size_t height = (channels > 0 && batch > 0) ? input[0][0].size() : 0;
    size_t width = (height > 0 && channels > 0 && batch > 0) ? input[0][0][0].size() : 0;
    ...
    for (int dim : target_shape) {
        ...
        if (minus_one_count == 1) {
            ...
            final_shape = std::vector<size_t>(target_shape.begin(), target_shape.end());
    ...
    // 提取目标维度
    size_t dim0 = final_shape[0];
    size_t dim1 = final_shape[1];
    size_t dim2 = final_shape[2];
